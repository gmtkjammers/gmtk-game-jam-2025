...
@export var node_attached_to_start : Node3D
@export var node_attached_to_end : Node3D
...
func update_start_end():
	...
	if node_attached_to_start != null:
		joints[0].node_b = node_attached_to_start.get_path()
	if node_attached_to_end != null:
		if fixed_end_point == false:
			joints.append(PinJoint3D.new())
			self.add_child(joints[-1])
			joints[-1].position = curve_points[-1] + position_buffer
			joints[-1].node_a = segments[-1].get_path()
		joints[-1].node_b = node_attached_to_end.get_path()
